...
class GastoUnisalud
{
    ...
    {
        $consulta = $this->db->prepare(
            "SELECT DISTINCT dependencia FROM gastos_unisalud
    ...
    {
        $consulta = $this->db->prepare('SELECT * FROM gastos_unisalud WHERE id = :id');
        $consulta->execute(['id' => $id]);
        $fila = $consulta->fetch();
    ...
    {
        $consulta = $this->db->prepare('DELETE FROM gastos_unisalud WHERE id = :id AND tipo_automatico IS NULL');
