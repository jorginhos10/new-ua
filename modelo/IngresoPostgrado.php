...
class IngresoPostgrado
{
    ...
    {
        $consulta = $this->db->prepare(
            "SELECT COALESCE(SUM(i.valor_total), 0) FROM ingresos_postgrado i
             WHERE i.anio_presupuestal_id = :anio_presupuestal_id
                AND NOT EXISTS (
                    SELECT 1 FROM peticiones_archivadas pa
     ...
     */
    public function enviarTodosBorrador(int $anioPresupuestalId, int $autogestionId, string $dependenciaDestinoNombre, int $rolDestinatarioId, array $dependenciasOrigen, ?int $usuarioDestinatarioId = null): array
    {
        ...
        $seleccion = $this->db->prepare("SELECT * FROM ingresos_postgrado WHERE $criterioWhere");
     ...
     * por su dueño en el módulo de Postgrado.
     */
    public function devolverABorrador(int $id): bool
    {
        $consulta = $this->db->prepare(
            "UPDATE ingresos_postgrado SET estado = 'borrador', rol_destinatario_id = NULL,
