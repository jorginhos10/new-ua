...
class IngresoUnisalud
{
    ...
    public function obtenerPorAnio(int $anioPresupuestalId): array
    {
        ...
        $consultaConceptos = $this->db->prepare(
            "SELECT * FROM ingresos_unisalud_conceptos WHERE ingreso_id IN ($marcadores) ORDER BY id ASC"
        );
    ...
    private const EXCLUIR_EXPEDIENTE_INGRESO_UNISALUD = "AND NOT EXISTS (
    ...
    {
        $consulta = $this->db->prepare(
            'SELECT COALESCE(SUM(valor_total), 0) FROM ingresos_unisalud
             WHERE anio_presupuestal_id = :anio_presupuestal_id ' . self::EXCLUIR_EXPEDIENTE_INGRESO_UNISALUD
        );
        $consulta->execute(['anio_presupuestal_id' => $anioPresupuestalId]);
    ...
    public function obtenerTotalPorAnioYDependencias(int $anioPresupuestalId, array $dependencias): float
    {
        ...
        $consulta = $this->db->prepare(
            'SELECT COALESCE(SUM(valor_total), 0) FROM ingresos_unisalud
             WHERE anio_presupuestal_id = :anio_presupuestal_id AND dependencia IN (' . implode(', ', $marcadores) . ') ' . self::EXCLUIR_EXPEDIENTE_INGRESO_UNISALUD
        );
    ...
    {
        $consulta = $this->db->prepare(
            "SELECT DISTINCT dependencia FROM ingresos_unisalud
    ...
    public function enviarTodosBorrador(int $anioPresupuestalId, string $dependenciaDestinoNombre, int $rolDestinatarioId, array $dependenciasOrigen, ?int $usuarioDestinatarioId = null): array
    {
        ...
        $seleccion = $this->db->prepare("SELECT * FROM ingresos_unisalud WHERE $criterioWhere");
        $seleccion->execute($parametrosBusqueda);
        $filas = $seleccion->fetchAll();

        if (!empty($filas)) {
            $consulta = $this->db->prepare(
                "UPDATE ingresos_unisalud
    ...
    {
        $consulta = $this->db->prepare(
            "UPDATE ingresos_unisalud SET estado = 'borrador', rol_destinatario_id = NULL,
    ...
    public function obtenerDependenciasSugeridas(): array
    {
        $consulta = $this->db->query(
            'SELECT DISTINCT dependencia FROM ingresos_unisalud ORDER BY dependencia ASC'
        );
    ...
    {
        $consulta = $this->db->prepare('SELECT * FROM ingresos_unisalud WHERE id = :id');
        ...
        $consultaConceptos = $this->db->prepare(
            'SELECT * FROM ingresos_unisalud_conceptos WHERE ingreso_id = :ingreso_id ORDER BY id ASC'
        );
    ...
    public function actualizar(int $id, array $cabecera, array $conceptos): bool
    {
        ...
        try {
            ...
            $this->db->prepare('DELETE FROM ingresos_unisalud_conceptos WHERE ingreso_id = :ingreso_id')
                ->execute(['ingreso_id' => $id]);

            $consultaConcepto = $this->db->prepare(
                'INSERT INTO ingresos_unisalud_conceptos (ingreso_id, concepto, cantidad, valor)
    ...
    {
        $consulta = $this->db->prepare('DELETE FROM ingresos_unisalud WHERE id = :id');
        $consulta->execute(['id' => $id]);
