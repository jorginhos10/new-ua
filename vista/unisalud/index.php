...
?>
    ...
    <div class="tarjeta">
        ...
        $barraRutaVolver = $modoEdicion
            ? ($volverEdicion !== '' ? $volverEdicion : 'index.php?ruta=unisalud&tab=' . $tab . '&anio_id=' . $anioSeleccionadoId)
            : null;
        $barraTextoVolver = ($modoEdicion && $volverEdicion !== '') ? 'Volver a Peticiones' : 'Volver a Unidad de Salud';
        require __DIR__ . '/../parciales/barra-modulo.php';
        ?>

        <div class="pestanas">
            <a href="index.php?ruta=unisalud&tab=ingresos" class="pestana<?= $tab === 'ingresos' ? ' activa' : '' ?>">Ingresos</a>
            <a href="index.php?ruta=unisalud&tab=egresos" class="pestana<?= $tab === 'egresos' ? ' activa' : '' ?>">Egresos</a>
            <?php if ($tab === 'ingresos'): ?>
        ...
        <?php if (count($aniosActivos) >= 2): ?>
            <form method="GET" action="index.php" class="form-filtro-anio">
                <input type="hidden" name="ruta" value="unisalud">
            ...
            <div class="acordeon-cuerpo">
                ...
                >
                    <table class="tabla-usuarios">
                        ...
                        <tbody>
                            <?php foreach ($egresosBorrador as $gasto): $filaEgresoUnisalud($gasto); endforeach; ?>
        ...
        $filaIngresoUnisalud = static function (array $ingreso) use ($anioSeleccionadoId): void {
            ?>
                    <tr>
                        <td class="columna-seleccion">
                        ...
                        <td class="celda-acciones">
                            ...
                            <div class="acciones-fila">
                                <a
                                    href="index.php?ruta=unisalud&tab=ingresos&anio_id=<?= (int) $anioSeleccionadoId ?>&editar_id=<?= (int) $ingreso['id'] ?>"
                                    class="boton-accion boton-accion-editar"
                                >Editar</a>
            ...
            <div class="acordeon-cuerpo">
                ...
                >
                    <table class="tabla-usuarios">
                        ...
                        <tbody>
                            <?php foreach ($ingresosBorrador as $ingreso): $filaIngresoUnisalud($ingreso); endforeach; ?>
    ...
    <div id="modal-gasto" class="modal-fondo<?= !empty($error) ? ' abierto' : '' ?>">
        <div class="modal-caja">
            ...
            <?php if ($tab === 'egresos'): ?>
            <form method="POST" action="index.php?ruta=unisalud&tab=egresos" class="form-necesidad">
                <input type="hidden" name="tab" value="egresos">
                <div class="campo">
            ...
            <?php else: ?>
            <form method="POST" action="index.php?ruta=unisalud&tab=ingresos" class="form-necesidad">
                <input type="hidden" name="tab" value="ingresos">
                <div class="campo">
                    <label for="anio_presupuestal_id">Año presupuestal *</label>
                    <select id="anio_presupuestal_id" name="anio_presupuestal_id" required>
    ...
    <div id="modal-enviar-todo-unisalud" class="modal-fondo">
        <div class="modal-caja">
            <div class="modal-cabecera">
                <h2>Enviar todos los ingresos y egresos</h2>
                <button type="button" id="boton-cerrar-modal-enviar-todo-unisalud" class="modal-cerrar" aria-label="Cerrar">&times;</button>
            </div>
            ...
            <form method="POST" action="index.php?ruta=unisalud&anio_id=<?= (int) $anioSeleccionadoId ?>" class="form-necesidad">
                <input type="hidden" name="accion" value="enviar_todo">
                <input type="hidden" name="anio_presupuestal_id" value="<?= (int) $anioSeleccionadoId ?>">

                <div class="campo">
                    <label for="enviar-todo-unisalud-destino_buscador">Enviar a la dependencia *</label>
                    <?php
                    $idPrefijoDependencia = 'enviar-todo-unisalud-destino-';
                    $nombreCampoDependencia = 'dependencia_destino';
                    $idBaseDependenciaOverride = 'enviar-todo-unisalud-destino';
                ...
                <div class="campo">
                    <label for="enviar-todo-unisalud-destinatario">¿A quién se enviará? *</label>
                    <select
                        id="enviar-todo-unisalud-destinatario"
                        class="selector-rol-destinatario"
                        data-campo-dependencia="enviar-todo-unisalud-destino"
                        data-campo-rol-oculto="enviar-todo-unisalud-rol"
                        data-campo-usuario-oculto="enviar-todo-unisalud-usuario"
                        required
                    >
                        <option value="">Selecciona a quién enviarlo</option>
                    </select>
                    <input type="hidden" id="enviar-todo-unisalud-rol" name="rol_destinatario_id">
                    <input type="hidden" id="enviar-todo-unisalud-usuario" name="usuario_destinatario_id">
                </div>
