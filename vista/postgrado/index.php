...
?>
    ...
    <div class="tarjeta">
        ...
        $barraGruposIconos = [
            ...
            'datos' => [
                [
                    ...
                    'tipo' => 'a',
                    'href' => 'index.php?ruta=postgrado-exportar-plantilla',
                ],
                [
                    'id' => 'boton-importar-postgrado',
                    'icono' => 'importar',
                ...
                [
                    'id' => 'boton-exportar-postgrado',
                    'icono' => 'exportar',
                    'etiqueta' => 'Exportar ingresos y gastos (.xlsx)',
                    'tipo' => 'a',
                    'href' => 'index.php?ruta=postgrado-exportar' . ($anioSeleccionadoId > 0 ? '&anio_id=' . $anioSeleccionadoId : ''),
                ],
            ],
        ];
        if ($modoEdicion) {
        ...
        $barraBotonEnviar = [
            ...
            'titulo_disabled' => $modoEdicion ? 'No disponible mientras editas' : 'Disponible cuando el total de egresos sea igual al total de ingresos de este ítem',
        ];
        $barraBuscar = $modoEdicion ? null : '[data-toggle-envio="postgrado"]';
        ...
        <?php if ($tab === 'egresos' && $anioSeleccionado): ?>
            <div class="progreso-presupuesto">
                ...
                <div class="barra-progreso">
                    <div class="barra-progreso-segmento costos" style="width: <?= number_format($pctCostos + $pctInversion, 2, '.', '') ?>%;"></div>
        ...
        $filaEgresoPostgrado = static function (array $gasto) use ($anioSeleccionadoId, $autogestionSeleccionadoId, $nombresMeses, $puedeAdministrarContribucion, $puedeAdministrarExcedentes, $permisosAutomaticosPorTipo): void {
            $mesesGasto = $gasto['meses'] !== ''
                ? array_map(static fn ($mes) => $nombresMeses[(int) $mes] ?? $mes, explode(',', $gasto['meses']))
                : [];
            $tieneEdicionLegado = ($gasto['tipo_automatico'] === 'contrib_postgrado' && $puedeAdministrarContribucion)
                || ($gasto['tipo_automatico'] === 'excedentes' && $puedeAdministrarExcedentes);
            $tieneEdicionPorPermiso = !$tieneEdicionLegado && ($permisosAutomaticosPorTipo[$gasto['tipo_automatico']] ?? false);
            $puedeGestionarFila = $gasto['tipo_automatico'] === null || $tieneEdicionLegado || $tieneEdicionPorPermiso;
            ?>
                    <tr>
                        ...
                        <td class="celda-acciones">
                            ...
                            <?php elseif ($gasto['tipo_automatico'] === 'contrib_postgrado' && $gasto['estado'] === 'borrador' && $puedeAdministrarContribucion): ?>
                            <button
                                type="button"
                                class="boton-accion boton-accion-editar boton-editar-egreso"
                                data-gasto="<?= htmlspecialchars(json_encode($gasto)) ?>"
                                title="Convertir en Gastos o Inversión"
                            >Editar</button>
                            <?php elseif ($gasto['tipo_automatico'] === 'excedentes' && $gasto['estado'] === 'borrador' && $puedeAdministrarExcedentes): ?>
                            <button
                                type="button"
                                class="boton-accion boton-accion-editar boton-editar-egreso"
                                data-gasto="<?= htmlspecialchars(json_encode($gasto)) ?>"
                                title="Convertir en Gastos o Inversión"
                            >Editar</button>
                            <?php elseif ($tieneEdicionPorPermiso && $gasto['estado'] === 'borrador'): ?>
                            <div class="acciones-fila">
                                <a
                                    href="index.php?ruta=postgrado&tab=egresos&anio_id=<?= (int) $anioSeleccionadoId ?>&autogestion_id=<?= (int) $autogestionSeleccionadoId ?>&editar_id=<?= (int) $gasto['id'] ?>"
                                    class="boton-accion boton-accion-editar"
        ...
        <div class="fila-chips-toggle solo-toggle">
            <button type="button" class="toggle-envio-boton es-borrador" data-toggle-envio-boton="postgrado" data-texto-borrador="<?= htmlspecialchars($textoToggleBorrador) ?>" data-texto-enviado="<?= htmlspecialchars($textoToggleEnviado) ?>"><?= htmlspecialchars($textoToggleBorrador) ?></button>
        </div>
        <div data-toggle-envio="postgrado" data-mostrando="borrador">
            <div class="panel-toggle-envio" data-panel-envio="borrador">
                ...
                >
                    <table class="tabla-usuarios">
                        ...
                        <tbody>
                            <?php foreach ($egresosBorrador as $gasto): $filaEgresoPostgrado($gasto); endforeach; ?>
            ...
            <div class="panel-toggle-envio oculto" data-panel-envio="enviado">
                <div class="panel-lotes-enviados">
                    ...
                    <?php endif; ?>
                    <?php
                    $rutaAccionLote = 'index.php?ruta=postgrado&tab=egresos&anio_id=' . (int) $anioSeleccionadoId . '&autogestion_id=' . (int) $autogestionSeleccionadoId;
                    $origenLote = 'gasto_postgrado';
        ...
        <div data-toggle-envio="postgrado" data-mostrando="borrador">
            <div class="panel-toggle-envio" data-panel-envio="borrador">
                ...
                >
                    <table class="tabla-usuarios">
                        ...
                        <tbody>
                            <?php foreach ($ingresosBorrador as $ingreso): $filaIngresoPostgrado($ingreso); endforeach; ?>
            ...
            <div class="panel-toggle-envio oculto" data-panel-envio="enviado">
                <div class="panel-lotes-enviados">
                    ...
                    <?php endif; ?>
                    <?php
                    $rutaAccionLote = 'index.php?ruta=postgrado&tab=ingresos&anio_id=' . (int) $anioSeleccionadoId . '&autogestion_id=' . (int) $autogestionSeleccionadoId;
                    $origenLote = 'ingreso_postgrado';
                    foreach ($lotesEnviados as $loteVista) {
                        require __DIR__ . '/../parciales/tarjeta-lote.php';
                    }
                    ?>
        ...
        <div class="modal-caja">
            ...
            <form method="POST" action="index.php?ruta=postgrado&anio_id=<?= (int) $anioSeleccionadoId ?>&autogestion_id=<?= (int) $autogestionSeleccionadoId ?>" class="form-necesidad">
                ...
                <div class="campo">
                    ...
                    <?php
                    $idPrefijoDependencia = 'enviar-todo-postgrado-destino-';
                    $nombreCampoDependencia = 'dependencia_destino';
                    $idBaseDependenciaOverride = 'enviar-todo-postgrado-destino';
                ...
                <div class="campo">
                    <label for="enviar-todo-postgrado-destinatario">¿A quién se enviará? *</label>
                    <select
                        id="enviar-todo-postgrado-destinatario"
                        class="selector-rol-destinatario"
                        data-campo-dependencia="enviar-todo-postgrado-destino"
                        data-campo-rol-oculto="enviar-todo-postgrado-rol"
                        data-campo-usuario-oculto="enviar-todo-postgrado-usuario"
                        required
                    >
                        <option value="">Selecciona a quién enviarlo</option>
                    </select>
                    <input type="hidden" id="enviar-todo-postgrado-rol" name="rol_destinatario_id">
                    <input type="hidden" id="enviar-todo-postgrado-usuario" name="usuario_destinatario_id">
                </div>
