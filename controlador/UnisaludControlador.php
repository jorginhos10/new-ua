...
class UnisaludControlador
{
    ...
    public function exportarPlantilla(): void
    {
        ...
        $hojaIngresos = [
            ...
            'filaEjemplo' => [
                ...
                'Ejemplo: aporte Unidad de Salud',
                '1',
                '1000000',
                '',
            ],
        ...
        $hojaGastos = [
            ...
            'filaEjemplo' => [
                ...
                'Ejemplo: dotación médica',
                $listasComunes['Rubros'][0] ?? '',
                'Ejemplo: insumos médicos',
                '1',
                '1000000',
                '',
                '1,2,3',
        ...
        $porcentajesModulo = $this->modeloPorcentaje->obtenerPorModulo('unisalud');
        ...
        $metadatos = [
            'plantilla' => 'autogestion-unisalud',
            'usuario_id' => (int) $_SESSION['usuario_id'],
            'usuario_nombre' => $catalogos['usuarioActual']['nombre'] ?? '',
        ];

        GeneradorXlsx::descargarPlantillaAutogestion('plantilla_unisalud.xlsx', $hojaIngresos, $hojaGastos, $validacionPorcentajes, $listasComunes, $metadatos);
        exit;
    }
    ...
    public function exportar(): void
    {
        ...
        GeneradorXlsx::descargarHojas('unisalud_' . $anioTexto . '.xlsx', $hojas);
        exit;
    }
    ...
    private function importar(): array
    {
        ...
        if (($metadatos['SPPI_Origen'] ?? '') !== GeneradorXlsx::FIRMA_PLATAFORMA || ($metadatos['SPPI_Plantilla'] ?? '') !== 'autogestion-unisalud') {
        ...
        foreach ($totalesPorGrupo as $clave => $grupo) {
            ...
            foreach ($grupo['categorias'] as $categoria => $totalCategoria) {
                ...
                if ($categoria === 'Excedentes') {
                    ...
                    // misma clave (año:dependencia) — solo funciona si ese ingreso también viene en
                    // este mismo archivo (ver importar(), más abajo); si no, se rechaza en vez de
                    // dejarlas caer silenciosamente sin insertarse en ningún lado.
                    if (!isset($gruposIngreso[$clave])) {
                        $errores[] = "Gastos, \"$dependenciaTexto\", categoría Excedentes: para reemplazar el excedente calculado, este mismo archivo debe incluir también la fila de Ingresos de \"$dependenciaTexto\" para este año.";
                        continue;
                    }
        ...
        // automática calculada del ingreso de esa misma clave (año:dependencia) — su suma ya se
        // validó arriba contra el % esperado.
        ...
        foreach ($gruposAfectados as $grupo) {
            $clave = $grupo['anio_presupuestal_id'] . ':' . $grupo['dependencia'];
    ...
    private function eliminarIngreso(): array
    {
        ...
        // la FK fk_gasto_unisalud_ingreso (ON DELETE SET NULL) pone ingreso_id a NULL en cuanto se
     ...
     *        Clave = tipo ('excedentes'). Ver el mismo mecanismo en ExtensionControlador.
     */
    private function regenerarAutomaticosDeIngreso(array $ingreso, array $lineasPersonalizadas = []): void
    {
        ...
        foreach ($lineas as $tipo => $info) {
            ...
            $definicion = $this->modeloDefinicionAutomatico->buscarConFallback(self::MODULO_AUTOGESTION, null, $tipo, $dependencia);
